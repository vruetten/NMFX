...
from .losses import (
    compute_batch_H_loss,
    compute_spatial_loss_coefficients,
    compute_W_loss,
)
...
def nmf(
    ...
    valid_init = ["random", "nndsvd"]
    if init not in valid_init:
        print("could not find intialization algorithm")
        init = "random"
    ...
    if (parameters.l2_space > 0) & (coordinates is not None):
        print("TAKING INTO ACCOUNT COORDINATES")
        if taus is not None:
            taus = initialize_taus(k)  # initialize
        spatial_loss_coefficients = compute_spatial_loss_coefficients(taus, coordinates)
    else:
        print("NOT TAKING INTO ACCOUNT COORDINATES")
    ...
    try:
        for i in range(parameters.max_iter):
            ...
            loss0 = compute_loss(X, W_prev, H_prev)
            ...
            if save_iter is not None:
                if i % save_iter == 0:
                    results = store_results(H_prev, W_prev, taus, i, log)
            ...
            if loss_batch < parameters.min_loss:
                print(statement)
                print("Fitting converged! Loss criteria reached")
                results = store_results(H_prev, W_prev, taus, i, log)
                np.save(save_path + f"_ite_{i:05d}.npy", results)
                break

            elif (i > 10) & (H_diff < parameters.min_diff):
                print(statement)
    ...
    loss = compute_loss(X, W_prev, H_prev)
    print(loss)
    ### project solution back into feasible space
    H = log1pexp(H_prev)  # don't apply last update - might be wors
    W = log1pexp(W_prev)
...
def store_results(H, W, taus, i, log):
